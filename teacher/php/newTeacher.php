    ...
    if (empty($_POST['username']) || empty($_POST['student_email'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($teacher_id) {
                ...
                $stmt2 = $conn->prepare("INSERT INTO instructor(instructor_email, userID) VALUES (?, ?)");

                // Bind the parameters
                $stmt2->bind_param("si", $_POST['student_email'], $teacher_id);

                // Execute the statement
                $stmt2->execute();

                // Redirect to the dashboard
                header("Location: /aceTrain/teacher/php/TeacherMain.php");
                session_write_close();
            }  else {
